...
#define TOP_FADE_START 0.88
#define TOP_FADE_EXP 0.5
...
#define DITHER_STRENGTH 1.0
    ...
    for(int s=0;s<2;++s){
        ...
        for(int i=0;i<W_LANES;++i){
            float activeLane = step(float(i), lanesF - 0.5);
            ...
            sum+=amp*lat*seg1*activeLane;
        }
    }
    // Wisps flow all the way to the top edge
    float span=smoothstep(-3.0,0.0,y);
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        // Emerging continuously from top without discrete mid-screen cutoff
        float cap=1.0; env*=cap;
    ...
    // Emerging from top: do not fade beam or wisps out before the top edge
    float topA=1.0;
    float L=a+b*topA;
