...
const EDGE_COPIES = 2;
const GLOW_LAYERS = [
  { blur: 6, opacity: 0.55, reach: 0.3 },
  { blur: 14, opacity: 0.35, reach: 0.6 },
  { blur: 36, opacity: 0.2, reach: 1 },
];
...
export const NeonBorder: React.FC<NeonBorderProps> = ({
    ...
    const frame = (now: number) => {
      ...
      if (s > 0) {
        ...
        const beat = step
          ...
          : (SLOWEST_CYCLE + ((FASTEST_CYCLE - SLOWEST_CYCLE) * (s - 1)) / 19) /
            4;
  ...
  // Compute radius: if rounded <= 50 treat as percentage of min dimension, else pixels
  const radius =
    rounded <= 50
